    ...
    handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
        if (!(M.chat === 'dm') && M.WAMessage.key.fromMe && M.WAMessage.status.toString() === '2') {
            ...
            */
            ...
            M.sender.username = this.client.user.name || this.client.user.vname || this.client.user.short || 'Offset hacker'
        ...
        if (user.ban) return void M.reply("Offset hacker Banned You from using commands.")
        ...
        if (command.config?.adminOnly && !M.sender.isAdmin)
            return void M.reply(`This command can only be used by admins`)
        try {
